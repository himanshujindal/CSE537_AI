...
This program implements the solution of the 4 gallon and 3 gallon jugs using DFS
I explore the possibilities from the empty set where both jugs are empty
and the do DFS on it till a solution (2 in jug 4) is obtained.
*/
...
int main()
{
    ...
    start_time = clock();
    stack <node> S;      //A stack to store the nodes generated
    ...
    S.push(root);  //Pushing the root into our stack and initialising
    AddVisited(root, Visited);
    memory++;
    while(!S.empty())
    {
         iterations++;
         node cur_node = S.top(); //Take the front from the stack
         ...
         cout<<"\nThis node is not a solution. "
             <<"\nGenerating children node and loading into stack.";
         S.pop();   //IF it is not the solution pop it and generate its children
         memory--;  //reduce memory since a node has been popped
         memory += Generate_Children(cur_node, S, Visited);    //Increment memory by the number of nodes added    
...
/*
Checks if the visited node is already present in the stack
Returns false if it is already prersent
Adds the node and returns true if it were not present
*/
...
/*
We take the node and the stack as the input and generate child on the following possibilities
1. We add water to the 4 gallon jug    Only possible if jug is not full
2. We add water to the 3 gallon jug    Only possible if jug is not full
3. We empty 4 gallon jug               Only possible if jug is not empty
4. We empty 3 gallon jug               Only possible if jug is not empty
...
{   
    ...
    if(a.jug_4 < 4)           //1. We add water to the 4 gallon jug    Only possible if jug is not full
    {
         ...
         {
              cout<<"\nFilling Jug 4-->Pushing element "<<b.jug_4<<" "<<b.jug_3<<" into the stack";
              S.push(b);
              i++;
         }
    }
    if(a.jug_3 < 3)            //2. We add water to the 3 gallon jug    Only possible if jug is not full
    {
         ...
         if(AddVisited(b, Visited) == true)
         {
              cout<<"\nFilling Jug 3-->Pushing element "<<b.jug_4<<" "<<b.jug_3<<" into the stack";
              S.push(b);
              i++;
         }
    }
    if(a.jug_4 > 0)            //3. We empty 4 gallon jug               Only possible if jug is not empty
    {
         ...
         if(AddVisited(b, Visited) == true)
         {
              cout<<"\nEmptying Jug 4-->Pushing element "<<b.jug_4<<" "<<b.jug_3<<" into the stack";
              S.push(b);
              i++;
         }
    }
    if(a.jug_3 > 0)             //4. We empty 3 gallon jug               Only possible if jug is not empty
    {
         ...
         if(AddVisited(b, Visited) == true)
         {
              cout<<"\nemptying Jug 3-->Pushing element "<<b.jug_4<<" "<<b.jug_3<<" into the stack";
              S.push(b);
              i++;
         }
    }
    if(a.jug_4 > 0 && a.jug_3 < 3)              //5. We transfer contetnts from 4 gallon to 3 gallon jug  Only possible if jug 4 not empty and jug 3 is not full
    {
         ...
         if(AddVisited(b, Visited) == true)
         {
              cout<<"\nJug 4 to Jug 3-->Pushing element "<<b.jug_4<<" "<<b.jug_3<<" into the stack";
              S.push(b);
              i++;
         }
    }
    if(a.jug_4 < 4 && a.jug_3 > 0)              //6. We transfer contents from 3 gallon to 4 gallon jug   Only possible if jug 3 not empty and jug 4 is not full
    {
         ...
         if(AddVisited(b, Visited) == true)
         {
              cout<<"\nJug 3 to Jug 4-->Pushing element "<<b.jug_4<<" "<<b.jug_3<<" into the stack";
              S.push(b);
              i++;
         }
    }
    cout<<endl<<i<<" elements pushed into the stack"<<endl;
    return i;
}
